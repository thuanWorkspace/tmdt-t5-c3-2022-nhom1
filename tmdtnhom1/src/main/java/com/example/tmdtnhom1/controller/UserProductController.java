...
public class UserProductController {
	...
	public ResponseEntity<UserProduct> orderProduct(@RequestBody UserProduct userProduct) {
		try{
			UserProduct user_Product = userProductService.orderProduct(new UserProduct(userProduct.getId_user(),
					userProduct.getId_product(), DateUtils.currentDate()));
	...
	/**
	 * số điểm sẽ tương ứng với tiền để mua sản phảm theo tỷ lệ
	 * người dùng có thể chọn mua bằng điểm nếu đủ số điểm tương ứng với giá tiền của sản phẩm
	 **/
	...
	public ResponseEntity<UserProduct> orderScoreProduct(@RequestBody UserProduct userProduct) {
		try{
			UserProduct user_Product = new UserProduct(userProduct.getId_user(),
					userProduct.getId_product(), DateUtils.currentDate());
